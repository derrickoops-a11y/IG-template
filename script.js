document.addEventListener('DOMContentLoaded', () => {
  ...
  function updateSpotifyEmbed() {
    ...
    if (spotifyMatch) {
      ...
      const customName = inputMusicName.value.trim();
      const startTime = parseInt(inputMusicTime.value.trim()) || 0;

      // Append start time based on Spotify widget URL params
      const timeParam = startTime > 0 ? `&t=${startTime}` : '';

      spotifyContainer.innerHTML = `<iframe src="https://open.spotify.com/embed/track/${trackId}?utm_source=generator${timeParam}" width="100%" height="152" frameBorder="0" allowfullscreen="" allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture" loading="lazy"></iframe>`;

      spotifyCustomization.style.display = 'block';
      renderMusicGroup.style.display = 'flex';

      if (customName) {
        renderMusic.innerHTML = `<span>${escapeHTML(customName)} &bull; Original Audio</span>`;
      } else {
        renderMusic.innerHTML = `<span>Spotify Track &bull; Original Audio</span>`;
      }
  ...
  inputFolder.addEventListener('change', (e) => {
    const files = Array.from(e.target.files).filter(f => f.type.startsWith('image/'));
    // Sort files by name so they appear in correct sequential order (e.g. 1.jpg, 2.jpg)
